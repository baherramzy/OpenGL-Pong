...
Pong::Pong(int screenWidth, int screenHeight) 
	...
	P1(10.0f, 10.0f), P2(float(screenWidth - P1.width - 10), 10.0f) {
	// Ball
	...
	ball = new Quad(ball_x, ball_y, ballSize, ballSize);
}
...
void Pong::updateBall() {
	...
	if (ball_x < P1.x + P1.width && ball_x > P1.x
		...
		ball_dir_y = ((ball_y - P1.y) / P1.height) - 0.2f;
	}
	...
	if (ball_x + ballSize > P2.x && ball_x + ballSize < P2.x + P2.width
		...
		ball_dir_y = ((ball_y - P2.y) / P2.height) - 0.2f;
	}
